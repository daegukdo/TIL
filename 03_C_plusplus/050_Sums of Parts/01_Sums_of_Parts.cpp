...
std::vector<unsigned long long> partsSum(const std::vector<unsigned long long>& ls)
{
	unsigned long long sumOfElems = 0;
	vector<unsigned long long>lsOfSumElems;
	int capacityOfLs = ls.capacity();

	sumOfElems = std::accumulate(ls.begin(), ls.end(), 0);
	lsOfSumElems.push_back(sumOfElems);

	for(int i = 0; i < capacityOfLs; i++)
	{
		sumOfElems -= ls[i];
		lsOfSumElems.push_back(sumOfElems);
	}
