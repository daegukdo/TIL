// https://modoocode.com/198#page-heading-6

// string class made by me
// 필요한 데이터
// --> 1. 문자열 데이터가 저장된 공간을 가리키는 포인터 ... 문자열의 길이 변경 시 쉽게 메모리를 해제하고 할당하기 위함
// --> 2. 문자열의 길이

#include <iostream>

// string.h 는 strlen 때문에 include 했는데, 
// 사실 여러분이 직접 strlen과 같은 함수를 만들어서 써도 됩니다.
#include <string.h>
...
public:
	...
	// find_from부터 시작하여 가장 가까운 str의 위치를 return / 없다면 -1
	int find(int find_from, MyString& str) const;
	int find(int find_from, const char* str) const;
	int find(int find_from, char c) const;

	// 원래의 객체와 str 사이에 사전적으로 나열하여 더 뒤에 오는 것이 큰 것
	// 처음부터 하나하나 비교 진행
	// 1이면 원래의 객체가 큰 것
	// -1이면 str이 더 큰 것
	// 0이면 같은 것
...
MyString& MyString::insert(int loc, const MyString& str) 
{
	...
	if (string_length + str.string_length > memory_capacity) 
	{
		if (memory_capacity * 2 > string_length + str.string_length)
			// 잦은 insert + 잦은 memory 초과를 방지하기 위함
			memory_capacity *= 2;
		else
			// 이제 새롭게 동적으로 할당을 해야 한다.
			memory_capacity = string_length + str.string_length;

		// 기존의 값이 있던 곳을 주소로 저장 ... 포인터 사용
		char* prev_string_content = string_content;

		// 새로운 char array 공간을 할당 ... 초과된 memory 포함하여 모두 담을 수 있는 공간
...
int main() 
{
	MyString str1("abqwds");
	MyString str2("abqjyue");
