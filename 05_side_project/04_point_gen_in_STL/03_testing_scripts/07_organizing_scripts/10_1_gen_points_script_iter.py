...
# test_nof                             = 8
# cadaver 2 case loop!
for nof in range(len(info_data_prefix)):

    # check file name
    file_name                              = point_processed_data_path + info_data_prefix[nof] + '_' + str(0) + '_points_processed'
    print(file_name)

    # load raw data
    point_init_plan                        = np.load(init_points_data_path + info_data_prefix[nof] + '_plan.npy')
    point40_on_STL                         = np.load(point40_data_path + info_data_prefix[nof] + 'points_40_selected.npy')
    each_point_data                        = np.load(STL_info_data_path + info_data_prefix[nof] + '_femur_each_point_data.npy')
    vertex_data                            = np.load(STL_info_data_path + info_data_prefix[nof] + '_femur_vertex_data.npy')
    plane_data                             = np.load(STL_info_data_path + info_data_prefix[nof] + '_femur_plane_data.npy')
    vector_data                            = np.load(STL_info_data_path + info_data_prefix[nof] + '_femur_vector_data.npy')
